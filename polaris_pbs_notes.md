...
trimmed baseline has less headroom.
...
- **2026-06-13 — THE 400-STEP PRODUCTION RUN IS GREEN (job 7198659,
  ...
  19:09:22, with the final `checkpoint-399` (4 × 2,011,703,942 B `mp_rank`
  ...
  complete checkpoint with ~5 min overhead), and full resume-from-checkpoint
  of the sharded distributed-optimizer state. The cumulative-across-requeues
  completion is real, not theoretical. **Checkpoint hygiene held throughout**:
  keep-newest-two-plus-final pruning ran live on each new save (12
  `checkpoint-<N>_listing_proof.txt` files on disk, 49–349, each a full
  `ls -laR` + `du`), final retained set 374 + 399, run dir 107 G, eagle 9.11 T
