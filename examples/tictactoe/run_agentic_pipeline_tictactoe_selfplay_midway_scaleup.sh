#!/bin/bash
# Midway SCALE-UP launcher (Qwen3-4B, 20 steps) — identical to
# run_agentic_pipeline_tictactoe_selfplay_midway.sh except for the config_name
# and the default output-dir label. Invoked from scripts/train_midway_scaleup.sbatch.
...
if [ -z "$ROLL_OUTPUT_DIR" ]; then
    ROLL_OUTPUT_DIR="/project/rcc/mehta5/MARSHAL/results/tictactoe_selfplay_midway_scaleup/$(date +%Y%m%d-%H%M%S)"
fi
...
python examples/start_agentic_pipeline.py \
  ...
  --config_name agentic_val_tictactoe_selfplay_midway_scaleup \
